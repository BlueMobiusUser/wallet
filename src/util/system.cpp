...
const char * const BITCOIN_CONF_FILENAME = "bluemobius.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //bluemobius-tx using stdin
...
{
    // Windows: C:\Users\Username\AppData\Roaming\BlueMobius
    // macOS: ~/Library/Application Support/BlueMobius
    // Unix-like: ~/.bluemobius
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BlueMobius";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/BlueMobius";
#else
    // Unix-like
    return pathRet / ".bluemobius";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty bluemobius.conf if it does not exist
