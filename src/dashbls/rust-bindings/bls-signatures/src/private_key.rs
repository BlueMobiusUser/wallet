...
impl PrivateKey {
    ...
    pub fn generate_bluemobius(rng: &mut StdRng) -> Result<Self, BlsError> {
    ...
    #[cfg(feature = "bluemobius_helpers")]
    pub fn generate_bluemobius_many(count: usize, rng: &mut StdRng) -> Result<Vec<Self>, BlsError> {
        (0..count)
            .into_iter()
            .map(|_| Self::generate_bluemobius(rng))
            .collect()
    }
