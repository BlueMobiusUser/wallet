...
static UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // This command is dangerous because it consumes 5 BMCOIN irreversibly.
    // If params are lost, it's very hard to bruteforce them and yet
    // users ignore all instructions on bluemobiuscentral etc. and do not save them...
...
{ //  category              name                      actor (function)         argNames
  //  --------------------- ------------------------  -----------------------  ----------
    /* BlueMobius features */
    { "bluemobius",               "getgovernanceinfo",      &getgovernanceinfo,      {} },
    { "bluemobius",               "getsuperblockbudget",    &getsuperblockbudget,    {"index"} },
    { "bluemobius",               "gobject",                &gobject,                {} },
    { "bluemobius",               "voteraw",                &voteraw,                {"tx_hash","tx_index","gov_hash","signal","outcome","time","sig"} },
