...
"""Test bluemobiusd with different proxy configuration.

Test plan:
- Start bluemobiusd's with different proxy configurations
- Use addnode to initiate connections
- Verify that proxies are connected to, and the right connection command is given
- Proxy configurations to test on bluemobiusd side:
...
class ProxyTest(BitcoinTestFramework):
    ...
    def node_test(self, node, proxies, auth, test_onion=True):
        ...
        # Note: bluemobiusd's SOCKS5 implementation only sends atyp DOMAINNAME, even if connecting directly to IPv4/IPv6
