...
source ./ci/bluemobius/matrix.sh
...
cd build-ci/bluemobius-$BUILD_TARGET
...
if [ "$DIRECT_WINE_EXEC_TESTS" = "true" ]; then
  ...
  wine ./src/test/test_bluemobius.exe
else
  if [ "$RUN_UNIT_TESTS_SEQUENTIAL" = "true" ]; then
    ./src/test/test_bluemobius --catch_system_errors=no -l test_suite
  else
