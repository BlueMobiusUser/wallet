...
The BlueMobius Core daemon can be configured to act as a trusted "border
router", implementing the bluemobius wire protocol and relay, making
...
The ZeroMQ feature in BlueMobius Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building bluemobiusd:
...
For instance:
    ...
    $ bluemobiusd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubrawtx=ipc:///tmp/bluemobiusd.tx.raw \
...
These options can also be provided in bluemobius.conf.
...
From the perspective of bluemobiusd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into bluemobiusd directly. Furthermore, no information is
...
using. BlueMobiusd appends an up-counting sequence number to each
