...
- [Development guidelines](#development-guidelines)
    - [General BlueMobius Core](#general-bluemobius-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
BlueMobius Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
BlueMobius Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/BlueMobius/BlueMobius/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_bluemobius
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_bluemobius --log_level=test_suite
$ valgrind -v --leak-check=full src/bluemobiusd -printtoconsole
...
# A coverage report will now be accessible at `./test_bluemobius.coverage/index.html`.
```
...
To profile a running bluemobiusd process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep bluemobiusd` -- sleep 60
```
...
BlueMobius Core can be compiled with various "sanitizers" enabled, which add
...
- [Main thread (`bluemobiusd`)](https://doxygen.bitcoincore.org/bitcoind_8cpp.html#a0ddf1224851353fc92bfbff6f499fa97)
...
- Net threads:
  ...
  - [ThreadSocketHandler (`d-net`)](https://doxygen.bitcoincore.org/class_c_connman.html#a765597cbfe99c083d8fa3d61bb464e34)
    : Sends/Receives data from peers on port 15658.
...
However, in open source software such as BlueMobius Core, where everyone uses
...
pay attention to for reviewers of BlueMobius Core code.

General BlueMobius Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: BlueMobius Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
be sent upstream, but bugfixes may also be prudent to PR against BlueMobius Core so that they can be integrated
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `bluemobius-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `bluemobius-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of BlueMobius Core, the wallet was always
...
- Use *invalid* BlueMobius addresses (e.g. in the constant array `EXAMPLE_ADDRESS`) for
